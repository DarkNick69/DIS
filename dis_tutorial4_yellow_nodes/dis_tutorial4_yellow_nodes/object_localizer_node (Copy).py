...
class ObjectLocalizerNode(Node):
    ...
    def __init__(self):
        ...
        self.ring_announce_client = self.create_client(
            RingColorAnnounce, '/ring_color_announce_service')
    ...
    def odom_callback(self, msg):
        pass  # odom used with TF2 transforms
        ...
        for det in detections.detections:
            ...
            try:
                ...
                if not is_duplicate:
                    ...
                    # announce ring color via service
                    if det.label == 'ring':
                        self.announce_ring(det.color)
    ...
    def announce_ring(self, color):
        """Call the ring color announce service."""
        if not self.ring_announce_client.wait_for_service(timeout_sec=3.0):
            self.get_logger().warn('Ring announce service not available!')
            return False

        request = RingColorAnnounce.Request()
        request.color = color

        future = self.ring_announce_client.call_async(request)
        rclpy.spin_until_future_complete(self, future)

        if future.result() is not None:
            return future.result().success
        else:
            self.get_logger().error('Ring announce service call failed!')
            return False
