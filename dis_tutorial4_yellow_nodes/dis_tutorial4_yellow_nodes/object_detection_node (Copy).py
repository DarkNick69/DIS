    ...
    def image_callback(self, msg):
        ...
        for color_name, ranges in color_ranges.items():
            ...
            for cnt in contours:
                ...
                # ignore small detections (far away objects or noise)
                if area < 50:
                    continue
                ...
                # aspect ratio check: rings look roughly square
                aspect_ratio = float(w) / float(h)
                if aspect_ratio < 0.5 or aspect_ratio > 2.0:
                    continue

                # HOLLOWNESS CHECK rings have a hole, cylinders and cubes don't
                # fill the contour and check if the center pixels have color
                cx_ring = x + w // 2
                cy_ring = y + h // 2
                r = max(1, min(w, h) // 6)
                y_start = max(0, cy_ring - r)
                y_end = min(mask.shape[0], cy_ring + r)
                x_start = max(0, cx_ring - r)
                x_end = min(mask.shape[1], cx_ring + r)
                center_region = mask[y_start:y_end, x_start:x_end]

                if center_region.size == 0:
                    continue

                # if center is filled with color, it's solid, we skip 
                center_fill = np.mean(center_region)
                if center_fill > 128:
                    continue
